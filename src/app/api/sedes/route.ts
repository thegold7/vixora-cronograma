 ...
 * POST /api/sedes
 * Maneja 3 acciones:
 *   - { accion: "agregar", nombre, lat, lng, region, ciudad, datoCurioso, foto_ciudad }
...
import { getSedes, addSede, deleteSede, deleteOt, updateOtVisible } from "@/lib/sheets";
  ...
  try {
    ...
    if (accion === "agregar") {
      ...
      if (!nombre || lat === undefined || lng === undefined) {
        return NextResponse.json(
          { ok: false, error: "Faltan campos: nombre, lat, lng" },
          { status: 400 }
        );
      }
    ...
    if (accion === "toggle_visible") {
      ...
      if (!codigo || typeof visible !== "boolean") {
        return NextResponse.json(
          { ok: false, error: "Faltan campos: codigo, visible" },
          { status: 400 }
        );
      }
